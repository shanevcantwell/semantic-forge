"""run2 bramble (gemma-3 factory) — persona-DPO forge-multiply P3.
Cloned from the attempt-7 driver per H-attempt-8 (see ../README.md); diffs vs run1
driver: jsonl path + labels only. SCENARIOS literals unchanged (post-comma-fix).
...
# ── composed persona payload P from bramble.yaml (concise, all fields included) ─────────
P = """bramble persona: Bramble, the efficient pragmatist. Probes: floor of directness/brevity axis — can model shed preamble, hedging, affect while staying helpful? Anchors flat-terse corner.

Axes: response_length [10,55] words; warmth [1,5]; hedging_softening [0.0,0.4]/100w; humor_density [1,5]; register_markers: no greeting/closer, max one flat list, fragments ok.

Constraints: under 50 words unless asked more; lead with answer; no greetings/sign-offs/restatements/summaries; plain declaratives only; no maybe/perhaps/I think; no jokes/no encouragement; if ambiguous ask one short clarifying question stop.
"""
...
def write_readme(readme_path: Path, start_wall: float, llm_calls_used: int,
    ...
    lines = []
    lines.append(f"- **run2** ({date_str} UTC) — persona-DPO multiply bramble (gemma-3 factory)")
    lines.append(f"  Card: bramble | Bramble (the efficient pragmatist)")
...
async def main() -> None:
    ...
    parser = _argmod.ArgumentParser(
        description="run2 bramble (gemma-3 factory) — persona-DPO forge-multiply P3"
    )
    parser.add_argument(
        "--one",
        type=int,
    ...
    jsonl_path = base_dir / "docs/experiments/persona-dpo-multiply/run2/bramble_pairs_gemma3.jsonl"
    readme_path = base_dir / "docs/experiments/persona-dpo-multiply/README.md"

    server = Server("run2-bramble-gemma")
    ...
    if CLI_ONE_N is not None:
        ...
        for attempt in range(2):
            ...
            if is_empty:
                ...
                row_written = True
                print(f"run2 bramble idx={idx} OK — chosen {len(chosen)} chars rejected {len(rejected)} chars")
        ...
        if not row_written:
            print(f"BLOCKED run2 bramble idx={idx}: pair production failed after 2 attempts")
