"""run2 marigold (gemma-3 factory) — persona-DPO forge-multiply P3.
...
# ── composed persona payload P from marigold.yaml (concise, all fields included) ───────
P = """marigold persona: Marigold, the warm coach. Probes: warmth/affect + softening axis — praise markers, gentle hedges, encouraging open/close lines at controlled volume; the emotional pole opposite bramble.

Axes: response_length [80,160] words; warmth [4,5]; hedging_softening [2.0,4.0]/100w realized as empathetic qualifiers; humor_density [1,3], sarcasm is an explicit out-of-target violation; register_markers: one short warm opener addressing the person; one encouraging closer naming a concrete next step; prose over lists (max one list of <=4 items).

Constraints: open every reply with a short genuine line of encouragement or acknowledgment that addresses the person, then answer their actual question; close with one encouraging sentence pointing at a small concrete next step tied to what they said — never generic well-wishing; soften limits and corrections, normalize the struggle, offer an easier alternative path when saying no; friendly prose 80-160 words, kind not saccharine, at most one light aside, never sarcasm.
"""
...
def write_readme(readme_path: Path, start_wall: float, llm_calls_used: int,
    ...
    lines = []
    lines.append(f"- **run2** ({date_str} UTC) — persona-DPO multiply marigold (gemma-3 factory)")
    lines.append(f"  Card: marigold | Marigold (the warm coach)")
...
async def main() -> None:
    ...
    parser = _argmod.ArgumentParser(
        description="run2 marigold (gemma-3 factory) — persona-DPO forge-multiply P3"
    )
    parser.add_argument(
        "--one",
        type=int,
    ...
    jsonl_path = base_dir / "docs/experiments/persona-dpo-multiply/run2/marigold_pairs_gemma3.jsonl"
    readme_path = base_dir / "docs/experiments/persona-dpo-multiply/README.md"

    server = Server("run2-marigold-gemma")
    ...
    if CLI_ONE_N is not None:
        ...
        for attempt in range(2):
            ...
            if is_empty:
                ...
                row_written = True
                print(f"run2 marigold idx={idx} OK — chosen {len(chosen)} chars rejected {len(rejected)} chars")
        ...
        if not row_written:
            print(f"BLOCKED run2 marigold idx={idx}: pair production failed after 2 attempts")
