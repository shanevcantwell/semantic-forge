...
- (NEW 2026-08-23) **In-session clock-regime disagreement:** an early `date -u` read this session returned ≈11:19 UTC; at authoring time the container, GitHub API Date header, and pc LM Studio host epoch all agree on ≈21:3x UTC — either a long real-time gap between turns or a host-clock step (mechanism unadjudicated from inside). Consequence: trust day-level dating and intra-regime ordering (all attempt `_generated_at` stamps are internally consistent); distrust fine-grained cross-turn diffs. External ground reachable through proxy: `curl -sI https://api.github.com | grep -i '^date'`. (2026-08-24 postscript: container + GitHub Date header agree at 16:19 UTC — regime stable at authoring of the block below.)

## Deltas since edition 2 (2026-08-24 UTC; head at writing d53a791 — verify before acting)

1. **Factory swapped to i9/gemma** (operator direction 2026-08-24): `inference-host:8081` serves `gemma-3-12b-it-IQ4_NL`; forge config model strings flipped (d53a791), extra_body kept as no-op guard. pc factory (`192.168.137.1:8081`, Qwen3.8) remains available, unused in this phase — a swap back is a decision, not a default, now that the triangle generator is pinned to gemma for P3-c interpretability (persona effect ⊥ generator identity). Unsloth provisioning for the i9 fine-tune lane (P5) started in-container same day; venv lands at `~/github/Unsloth/.venv`.
2. **Qwen bramble dataset superseded:** renamed `run1/bramble_pairs_run1_qwen38-superseded.jsonl`; triangle regenerated under one generator.
3. **H-gemma-think FALSIFIED** — no thinking-budget issue on this gemma serve; fenced JSON already handled by forge's `_extract_json`. Multiply README H-attempt-8 block carries the probe record.
4. **run2 state at halt:** bramble 5/5 rows banked under gemma (chosen<rejected 5/5, median chosen 116 chars); vex idx0 banked; HALTED at vex idx=1 on delta-5's SK teardown defect via the driver retry idiom (P2' pre-committed escalation — first production exposure). Open operator fork in the multiply README attempt-8 record: strict repair-first vs single-shot re-entry. 9 invocations outstanding (vex idx1–4 + marigold ×5).
5. **SK-teardown issue filed** with run2 as second-exposure evidence (see board); gates P4-scale multi-call runs per delta 5.
6. **Instrumentation gap banked:** --one error path does not persist the `_isError` raw payload — vex idx=1 attempt-1 failure cause is unadjudicated from disk; fix belongs with the single-shot driver edit if option (b) is chosen.
